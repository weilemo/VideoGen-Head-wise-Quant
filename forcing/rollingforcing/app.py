...
def _load_pipeline(config_path: str, checkpoint_path: Optional[str], use_ema: bool) -> torch.nn.Module:
    ...
    wan_dir = WAN_MODEL_ROOT
    if not os.path.isdir(wan_dir):
        raise gr.Error(
            f"Wan2.1-T2V-1.3B not found at '{WAN_MODEL_ROOT}'."
        )
...
def main():
    ...
    parser.add_argument('--config_path', type=str, default=str(DEFAULT_MODEL_CONFIG_PATH),
                        help='Path to the model config')
    parser.add_argument('--checkpoint_path', type=str, default=str(DEFAULT_ROLLING_FORCING_CKPT),
